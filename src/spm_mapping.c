...
static void get_map_file(int i, const mxArray *ptr, MAPTYPE *maps)
{
    ...
    {
        free_maps(maps,i+1);
        mexErrMsgTxt("Cannot find fname.");
    }
    if (mxIsChar(tmp))
    {
#ifdef SPM_WIN32
        ...
        if ((fd = open(buf, O_RDONLY)) == -1)
        {
            mxFree(buf);
            free_maps(maps,i+1);
            mexErrMsgTxt("Cannot open image file.");
        }
        if (fstat(fd, &stbuf) == -1)
        {
            (void)close(fd);
        ...
        mxFree(buf);
        if (hFile == NULL)
            mexErrMsgTxt("Cannot open file.  It may be locked by another program.");
        ...
        if (hMapping == NULL)
            mexErrMsgTxt("Cannot create file mapping.  It may be locked by another program.");
        ...
        if (maps[i].addr == NULL)
            mexErrMsgTxt("Cannot map view of file.  It may be locked by another program.");
        ...
        if (maps[i].addr == (void *)-1)
        {
            ...
            mxFree(buf);
            free_maps(maps,i+1);
            mexErrMsgTxt("Cannot map image file.");
        }
        mxFree(buf);
#endif
    }
